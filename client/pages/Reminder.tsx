...
const allReminders: DayReminders[] = [
  {
    date: "2025/11/15",
    title: "2025 / 11 / 15",
    groups: [
      {
        ...
        className: "수업 1",
        color: "#FFC374",
        reminders: [{ id: "15-1-1", title: "리마인더 예시", description: "상세내용", completed: false }],
      },
    ],
  },
  {
    ...
    groups: [
      {
        ...
        color: "#FFC374",
        reminders: [
          { id: "1-1", title: "저번 시간 숙제", description: "상세내용", completed: true },
          { id: "1-2", title: "프린트 챙겨오기", description: "상세내용", completed: true },
          { id: "1-3", title: "형성평가 X차시", description: "상세내용", completed: false },
        ],
      },
      {
        classId: "2",
        className: "수업 2",
        color: "#4AD2C9",
        reminders: [{ id: "2-1", title: "리마인더 1", description: "상세내용", completed: false }],
      },
    ],
  },
  {
    ...
    groups: [
      {
        ...
        className: "수업 4",
        color: "#536EFF",
        reminders: [{ id: "4-1", title: "리마인더 2", description: "상세내용", completed: false }],
      },
    ],
  },
  {
    ...
    groups: [
      {
        ...
        className: "수업 3",
        color: "#C44EFB",
        reminders: [{ id: "3-1", title: "과제 제출", description: "상세내용", completed: false }],
      },
    ],
  },
  {
    ...
    groups: [
      {
        ...
        className: "수업 5",
        color: "#7CED91",
        reminders: [{ id: "22-5-1", title: "리마인더 예시", description: "상세내용", completed: false }],
      },
    ],
  },
];
...
export default function Reminder() {
  ...
  const [reminders, setReminders] = useState<DayReminders[]>(allReminders);

  const currentDay = reminders[currentDateIndex];

  const handleToggleComplete = (groupId: string, reminderId: string) => {
    setReminders((prev) =>
      prev.map((day) => ({
        ...day,
  ...
  const handleEdit = (classId: string, reminderId: string) => {
  ...
  const handleNext = () => {
    if (currentDateIndex < reminders.length - 1) {
      setCurrentDateIndex(currentDateIndex + 1);
    }
  };
        ...
        {currentDay && (
          <div>
            ...
            {currentDay.groups.map((group) => (
              <div key={group.classId} className="mb-6">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <button
                    type="button"
                    onClick={() => group.reminders[0] && handleEdit(group.classId, group.reminders[0].id)}
                  >
                    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor" style={{ color: group.color }}>
                ...
                <div className="space-y-3">
                  {group.reminders.map((reminder) => (
                    <label key={reminder.id} className="flex items-start gap-3 cursor-pointer">
                      <button
                        type="button"
                        onClick={() => handleToggleComplete(group.classId, reminder.id)}
                        className={`mt-0.5 w-5 h-5 border-2 border-slate-900 rounded flex items-center justify-center ${
                          reminder.completed ? "bg-slate-900" : "bg-white"
                        }`}
                      >
                        {reminder.completed && (
                      ...
                      <div className="flex-1">
                        <span className={reminder.completed ? "line-through text-slate-600" : "text-[#010618]"}>
                          {reminder.title}
                        </span>
                        <p
                          className={`text-xs mt-1 ${
                            reminder.completed ? "text-slate-500 line-through" : "text-slate-500"
                          }`}
                        >
