...
const isBoardTab = (value: string): value is BoardTab => ["공지사항", "행사", "자료실"].includes(value as BoardTab);
...
export default function BoardDetail() {
  ...
  const currentIndex = useMemo(() => posts.findIndex((item) => item.id === post.id), [posts, post.id]);
    ...
    <div className="min-h-screen bg-white pb-24 flex flex-col">
      ...
      <div className="flex-1 px-6 py-6 space-y-6">
        <div>
          ...
          <h3 className="text-2xl font-bold text-[#010618] mt-2">{post.title}</h3>
        ...
        <div className="text-sm leading-6 text-[#010618] whitespace-pre-line">{post.content}</div>

        {post.attachment && (
          <div className="border border-slate-200 rounded-2xl px-4 py-3">
            <p className="text-sm font-semibold text-[#010618] mb-1">첨부파일</p>
            <a href={post.attachment} className="text-sm text-[#80B3FF] underline underline-offset-2" target="_blank" rel="noreferrer">
